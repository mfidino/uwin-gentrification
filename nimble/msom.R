...
msom_configure$addSampler(
  target = c("a_community", "a_species_re"),
  type = "RW_block",
  control = list(adaptInterval = 1000)
)

msom_configure$removeSampler(
  c("b_community", "b_species_re", 
    "b_species_city_re", "theta_psi", "theta_species")
)

msom_configure$addSampler(
  target = c("b_community", "b_species_re", 
             "b_species_city_re", "theta_psi_re",
             "theta_species_re"),
  type = "RW_block",
  control = list(adaptInterval = 1000)
)
...
msom_configure$removeSampler(
  c("c_community", "c_species_re", "c_species_city_re")
)

msom_configure$addSampler(
  target = c("c_community", "c_species_re", "c_species_city_re"),
  type = "RW_block",
  control = list(adaptInterval = 1000)
)
