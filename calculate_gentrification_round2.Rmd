...
# read in the imperv data
imperv <- read.csv("./data/imperv.csv")

to_save <- dplyr::inner_join(
  to_save[,c("City", "Site", "gentrifying", "vulnerable")],
  imperv[,c("city", "site", "mean_19")],
  by = c("City" = "city", "Site" = "site")
)
...
all_sites <- dplyr::bind_rows(gent_list)

all_sites <- all_sites[,-4]

all_tracts <- dplyr::bind_rows(gent_list_poly)

sf::st_write(
  all_sites,
  "./sander_all_data/sites.shp"
)

sf::st_write(
  all_tracts,
  "./Sander_all_data/tracts/tracts.shp"
)

# Jackson
dir.create("./gent_files/Jackson", recursive = TRUE)
dir.create("./gent_files/Jackson/sites")
dir.create("./gent_files/Jackson/tracts")

tmp_sites <- all_sites[all_sites$City == "jams",]
sf::st_write(
  tmp_sites,
  "./gent_files/Jackson/sites/sites.shp"
)

write.csv(as.data.frame(tmp_sites),
          "./gent_files/Jackson/sites/sites_df.csv",
          row.names = FALSE)

tmp_tracts <- all_tracts[all_tracts$City == "jams",]
sf::st_write(
  tmp_tracts,
  "./gent_files/Jackson/tracts/tracts.shp"
)
tiff("./gent_files/Jackson/map.tiff",
     height = 6, width = 6, units = "in",
     res = 600, compression = "lzw")
plot(tmp_tracts["gentrifying"],reset = FALSE)
gsites <- tmp_sites$gentrifying == TRUE
points(sf::st_coordinates(tmp_sites)[gsites,], pch = 21, bg = "purple", cex = 1.2)
points(sf::st_coordinates(tmp_sites)[!gsites,], pch = 21, bg = "white", cex = 1.2)
dev.off()
...
tmp_sites <- all_sites[all_sites$City == tcc,]
sf::st_write(
  tmp_sites,
  paste0(
    "./gent_files/",
...
write.csv(
  as.data.frame(tmp_sites),
  paste0(
    "./gent_files/",
    tc,"/sites/sites_df.csv"
  ),
  row.names = FALSE
)
tmp_tracts <- all_tracts[all_tracts$City == tcc,]
sf::st_write(
  tmp_tracts,
    paste0(
    "./gent_files/",
...
all_tracts$area <- sf::st_area(all_tracts)
...
test$gentrifying[is.na(test$gentrifying)] <- 2


test <- all_tracts %>% 
  dplyr::group_by(City) %>% 
  #dplyr::filter(!is.na(gentrifying)) %>% 
  dplyr::summarise(
    prop = sum(gentrifying) / length(gentrifying)
  ) %>% 
  data.frame() %>% 
  dplyr::select("City", "prop")
...
for(i in 1:length(test)){
  my_results$prop_gent[i] <- as.numeric(test[[i]]$ar[
    test[[i]]$gentrifying == 1
  ] /sum(test[[i]]$ar)) 
    #sum(test[[i]]$ar[test[[i]]$gentrifying != 2]))
}
